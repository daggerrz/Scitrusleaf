...
class ClMessageEncoder extends OneToOneEncoder {
  ...
  def encode(ctx: ChannelHandlerContext, channel: Channel, msg: Any) = {
    ...
    import buf._
    writeByte(Protocol.TYPE_VERSION)
  ...
  private[protocol] def encodeAsMessage(msg: AsMessage) = {
    ...
    for (f <- msg.fields) { FieldCodec.encode(f, buf) }
    for (f <- msg.ops) { OpCodec.encode(f, buf) }
    buf
  }
}

class ClMessageDecoder extends OneToOneDecoder {
  def decode(ctx: ChannelHandlerContext, channel: Channel, msg: Any) = {
    ...
    messageType match {
      case Protocol.TYPE_INFO => parseInfoMessage(header, frame)
      case Protocol.TYPE_MESSAGE => parseAsMessage(header, frame)
      case other =>
        buf.readSlice(header.messageLength.toInt)
  ...
  def parseAsMessage(header: ProtocolHeader, buf: ChannelBuffer) = {
    ...
    n = header.fieldCount
    while (i < n)  {
      fields(i) = FieldCodec.decode(buf)
      i += 1
    }
...
private[protocol] object FieldCodec {
...
private[protocol] object OpCodec {
...
private[protocol] object ClWireFormat {
  /**
